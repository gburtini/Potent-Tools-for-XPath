...
describe('evaluators', () => {
  describe('evaluateXPath', () => {
    it('should return STRING_TYPE from a simple DOM query.', () => {
      const xpath = evaluators.evaluateXPath(document, '/html/body/div/div[1]', document, XPathResult.STRING_TYPE);
      expect(xpath).to.be.equal('Hello world.', 'Expected inner string to be returned');
    });

    it('should return Node from a simple DOM query.', () => {
      const xpath = evaluators.evaluateXPath(document, '/html/body/div/div[1]', document);
      expect(xpath).to.be.deep.equal([childDiv1], 'Expected array of matching elements to be returned');
    });
  });
  ...
  describe('getRuleMatchingElements', () => {
    it('should support a similar lookup by CSS rule', () => {
      ...
      const result = evaluators.getRuleMatchingElements({ selectorText: '.test' }, document);
      expect(result).to.have.length(1);
      expect(result[0].className).to.be.eq('test');
    })
  });
